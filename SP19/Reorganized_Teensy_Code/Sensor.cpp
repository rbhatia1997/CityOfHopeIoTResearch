...
void Sensor::init(void){
    ...
    for( int imu = 0; imu < NUM_IMUS; imu++){
        Serial.print("Checking IMU#: "); Serial.println(imu);
        ...
        config_imu_settings(int imu);
        
        // INITIALIZE FILTERING METHOD (OR DO IN INO FILE)

        if (!IMU_List[imu].begin()){
            Serial.print("Failed to communicate with IMU #: "); Serial.println(imu);
...
void Sensor::cal_ag(bool stop_after){
    // need to implement
}

void Sensor::cal_m(bool stop_after){
    // need to implement
}
...
void Sensor::apply_calibrations(void){
    ...
    for( int imu = 0; imu < NUM_IMUS; imu++){
        ...
        mag_data[imu][0] = mx * mag_scaling[0][0] + my * mag_scaling[0][1] + mz * mag_scaling[0][2];
        mag_data[imu][1] = mx * mag_scaling[1][0] + my * mag_scaling[1][1] + mz * mag_scaling[1][2];
        mag_data[imu][2] = mx * mag_scaling[2][0] + my * mag_scaling[2][1] + mz * mag_scaling[2][2];
    }
}
