...
void setup() {
  ...
  Serial.begin(9600);
  pinMode(PIN_BOI, OUTPUT);        //Sets digital pin 2 as output pin
  while (!Serial) {
  ...
  void loop()
  {
    if (mySerial.available() > 0) // Send data only when you receive data:
    {
      ...
      if (data == '1') {          //Checks whether value of data is equal to 1
        digitalWrite(PIN_BOI, HIGH);  //If value is 1 then LED turns ON
        mySerial.print("Light's on!");
      }
      else if (data == '0') {      //Checks whether value of data is equal to 0
        digitalWrite(PIN_BOI, LOW);   //If value is 0 then LED turns OFF
